...
while [ 1 ]
do	
	TEST="$(docker run --rm --entrypoint=\"/cpp-nekonium/build/test/createRandomStateTest\" ethereum/cppjit-testrunner)"
	# echo "$TEST"
	
	# test pynekonium
	OUTPUT_PYTHON="$(docker run --rm nekonium/python-testrunner --notrace <<< "$TEST")"
	RESULT_PYTHON=$?

	# test go
	OUTPUT_GO="$(docker run --rm nekonium/go-testrunner "$TEST")"
	RESULT_GO=$?
	
	# test cpp-jit
	OUTPUT_CPPJIT="$(docker run --rm nekonium/cppjit-testrunner "$TEST")"
	RESULT_CPPJIT=$?
