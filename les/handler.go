...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Nekonium protocol")
	...
	log.Info("Light Nekonium protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light Nekonium peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Nekonium message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Nekonium message arrived", "code", msg.Code, "bytes", msg.Size)
