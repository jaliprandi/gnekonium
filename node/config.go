...
import (
	...
	"github.com/nekonium/go-nekonium/accounts/usbwallet"
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gnekonium", keep that.
	if name == "gnekonium" || name == "gnekonium-testnet" {
		name = "Gnekonium"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gnekonium" instances.
...
func (c *Config) resolvePath(path string) string {
	...
	// by gnekonium 1.4 are used if they exist.
	if c.name() == "gnekonium" && isOldGethResource[path] {
		oldpath := ""
		if c.Name == "gnekonium" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	default:
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-nekonium-keystore")
		ephemeral = keydir
	}
	if err != nil {
		return nil, "", err
